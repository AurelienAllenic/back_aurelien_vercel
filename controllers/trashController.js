...
exports.getAllTrashedItems = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Erreur lors de la récupération de la corbeille", error: err.message });
  }
};
...
exports.restoreItem = async (req, res) => {
  try {
    ...
    if (!trashItem) return res.status(404).json({ message: "Élément introuvable dans la corbeille" });

    const { entityType, data, originalId } = trashItem;
    const Model = modelsMap[entityType];
    if (!Model) return res.status(400).json({ message: "Type d'entité inconnu : " + entityType });

    // On vérifie que l'élément n'existe pas déjà
    const exists = await Model.findById(originalId);
    if (exists) return res.status(409).json({ message: `${entityType} existe déjà avec cet ID` });
    ...
    res.status(500).json({ message: "Erreur lors de la restauration", error: err.message });
  }
};
...
exports.permanentlyDeleteItem = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Erreur lors de la suppression définitive", error: err.message });
  }
};
